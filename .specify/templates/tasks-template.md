...
<!--
  ...
  - GOVERNANCE RULES from .specify/memory/governance-rules.md (MANDATORY)
...
- [ ] TGOV-01 [GOV] Create ADR for this feature: `docs/adrs/NNNN-[feature-name].md`
  - Document: Context (why), Decision (what), Consequences (trade-offs)
  - Use existing ADRs as format reference
...
- [ ] TGOV-02 [GOV] Verify new code placement follows Clean Architecture:
  ...
  - Domain entities (pure logic) → `src/domain/*.entity.ts`
  - Repository interfaces → `src/domain/*.repository.ts`
  - Use cases → `src/application/*.usecase.ts`
  - Adapters/CLI/API → `src/infrastructure/*`
...
- [ ] TGOV-05 [GOV] Verify ADR content matches implementation intent
